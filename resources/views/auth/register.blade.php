<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            <x-input-label for="nis" :value="__('NIS (For Students Only)')" />
            <x-text-input id="nis" class="block mt-1 w-full" type="text" name="nis" :value="old('nis')" />
            <x-input-error :messages="$errors->get('nis')" class="mt-2" />
        </div>

        <!-- Class (Hanya untuk Siswa) -->
        <div class="mt-4">
            <x-input-label for="class" :value="__('Class (For Students Only)')" />
            <x-text-input id="class" class="block mt-1 w-full" type="text" name="class" :value="old('class')" />
            <x-input-error :messages="$errors->get('class')" class="mt-2" />
        </div>

        <!-- Major (Hanya untuk Siswa) -->
        <div class="mt-4">
            <x-input-label for="major" :value="__('Major (For Students Only)')" />
